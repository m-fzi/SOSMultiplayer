...
typealias MyPlayer = LocalNetworkPlayer
typealias OpponentPlayer = LocalNetworkPlayer
...
distributed actor LocalNetworkPlayer: GamePlayer {
    ...
    init(team: CharacterTeam, model: GameViewModel, actorSystem: ActorSystem) {
        self.team = team
        self.model = model
        self.actorSystem = actorSystem
    }

    distributed func makeMove() async -> GameMove {
    ...
    distributed func makeMove(at position: Int) async -> GameMove {
        let move = GameMove(
            playerID: id,
            position: position,
            team: team,
    ...
    distributed func opponentMoved(_ move: GameMove) async throws {
        do {
    ...
    distributed func startGameWith(opponent: OpponentPlayer, startTurn: Bool) async {
